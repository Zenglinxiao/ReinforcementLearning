...
class Runner:
    ...
    def step(self):
        observation = self.environment.observe()#get actual state
        action = self.agent.act(observation)#get the action to take for the given state
        (reward, stop) = self.environment.act(action)# do the action, and return the reward
        observation = self.environment.observe()#get the state after the action
        self.agent.reward(observation, action, reward)# update according to the (state,action,reward)
